...
<?php $this->beginPage() ?>
    ...
    <nav class="navbar navbar-expand-sm">
        ...
        <form class="form-inline mx-auto" id="mainHeaderSearch" action="<?= \yii\helpers\Url::to(['/site/search']) ?>" method="get">
            <input class="form-control mr-md-0" type="search" name="search" id="search" placeholder="Поиск" aria-label="Search">
        ...
        <ul class="navbar-nav">
            ...
            <li class="nav-item">
                <a class="nav-link" href="<?= \yii\helpers\Url::toRoute(['site/browse']) ?>">Аниме</a>
            </li>

            <?php if (!Yii::$app->user->isGuest): ?>
            <?php if(Yii::$app->user->identity->isAdmin()): ?>
            <li class="nav-item">
                <a class="nav-link" href="<?= \yii\helpers\Url::toRoute(['/admin']) ?>">Админ-панель</a>
            </li>
            <?php endif; ?>
            <?php endif; ?>
            ...
            <?php if (Yii::$app->user->isGuest):
                ...
                <li class='nav-item dropdown dropleft'>
            	                <a class='nav-link' href='#' data-toggle='dropdown'>
                                    <?=  Html::img(Yii::$app->user->identity->getImage(),['class'=>'userIcon','width'=>'60px','alt'=>'userImg']) ?>
            	                </a>
            	                <div class='dropdown-menu'>
            	                    <a class='dropdown-item disabled' style='color:silver; text-transform:lowercase;' href='#'>
            	                    <?=  Yii::$app->user->identity['username']?></a>
            	                    <a class='dropdown-item' style='color:#fff;' href='<?= \yii\helpers\Url::to(['site/account']) ?>'>Аккаунт</a>
